...
When I have multiple promotional certificates with earlier certs active and latter certs used, and I try to book a promo cert round-trip pnr with outbound date before last unused cert and inbound date after the last unused cert,
Then I see an Oops message that the inbound date I selected is unavailable for the certificate

Scenario: RR Member views promotions, tries to book a pnr with outbound date before last unused cert and inbound date after the last unused cert, sees Oops message on select flights page
...
Then I see an Oops messages from select flight page indicating that The return date you selected is unavailable with this certificate. Please select another date.
